...
class BaseDataSet1(Dataset):
    ...
    # 自定义图片数组，数据类型一定要转为‘uint8’,不然transforms.ToTensor()不会归一化     
        if self.val:
            ...
            center_image, label = self._augmentation(center_image, label)
        ...
        if self.return_id:
            return  self.normalize(self.to_tensor(center_image)),label, image_id
        return self.normalize(self.to_tensor(center_image)),label
